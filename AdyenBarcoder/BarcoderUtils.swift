...
func isBigEndianFromMandatoryByteOrderFirstCharacter(_ format:String) -> Bool {
    ...
    guard format.isEmpty == false else { assertionFailure("empty format"); return false }
    
    let firstChar = format[format.startIndex]
...
func numberOfBytesInFormat(_ format:String, dataLength: Int) -> Int {
    ...
    while mutableFormat.isEmpty == false {
        ...
        if c == "*" {
            numberOfBytes += (dataLength - numberOfBytes)
            n = 0
            continue
        }
...
func formatDoesMatchDataLength(_ format:String, data:Data) -> Bool {
    ...
    let sizeAccordingToFormat = numberOfBytesInFormat(format, dataLength: dataLength)
    ...
    while mutableFormat.isEmpty == false {
        ...
        for _ in 0..<max(n,1) {
            ...
            case "H":
                ...
                o = isBigEndian ? UInt16(bigEndian: r) : r
            case "i":
                fallthrough
            case "l":
            ...
            case "L":
                ...
                o = isBigEndian ? UInt32(bigEndian: r) : r
            case "q":
